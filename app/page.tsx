...
export default async function Home() {
  ...
  return (
   <div className='flex flex-col'>
    ...
    <ul className="pt-8 ">
      {data.map((post)=>(
      <Card key={post._id} className="shadow-md hover:scale-95 duration-100 transition-all shadow-gray-600 flex mb-4 p-6 justify-between items-center bg-teal-900/30">
        <div className="w-[45%]">
          <p className=" font-medium text-gray-400 text-base leading-6">{new Date(post._createdAt).toISOString().split('T')[0]}</p>
        </div>
          {/* right div */}
        <Link href={`/post/${post.slug.current}`} prefetch className="w-[55%] cursor-pointer">
          <div className=" gap-y-2 flex flex-col justify-center  ">
              <h3 className=" text-xl sm:text-2xl font-bold leading-8 tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-blue-500 via-blue-300 to-blue-200">{post.title}</h3>
